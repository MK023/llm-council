...
def stage1_responses(
    ...
    for model in VOTER_MODELS:
        try:
            r = client.call(model, messages, MAX_TOKENS_STAGE_1)
...
def stage2_rankings(
    ...
    for voter in VOTER_MODELS:
        try:
            result = client.call(voter, messages, MAX_TOKENS_STAGE_2)
        except OpenRouterError as exc:
            rankings.append(
                RankingResult(
                    voter=voter,
...
def stage3_synthesis(
    ...
    messages = [{"role": "user", "content": prompt}]
    return client.call(CHAIRMAN_MODEL, messages, MAX_TOKENS_STAGE_3)
