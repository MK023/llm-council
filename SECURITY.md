...
| **LLM02** | Sensitive Information Disclosure | **Mitigated.** Per-request ZDR routing with `data_collection: deny`, fail-closed. Server-side PII redaction (email/phone/SSN/card). Telemetry carries identifiers only — never the question or the answers, enforced by `tests/test_stages.py::TestTelemetryPrivacy`. The fields added in 2026-08 (`generation_id`, `finish_reason`) are of the same kind: an opaque completion id and a stop reason, both useless to anyone who cannot already query the account. The API key is redacted in `__repr__`, never logged, and a test asserts it never reaches stdout/stderr. **`question_hash` is a truncated unsalted SHA-256** — widened from 32 to 64 bits on 2026-09-01 — and the honest limit is that a truncated unsalted digest can be *confirmed* by anyone already holding a candidate question. It reaches exactly two places, and the first draft of this row named a third that does not exist: the value never leaves `emit()`, so it lives only in the stderr record — Marco's terminal, and the public log of the weekly E2E, where the question is public anyway because it is written in `e2e.yml`. What travels to OpenRouter and on to Langfuse is `session_id`, a `uuid4`. Against an exposure that small, a salt would be a bad trade: it would close the confirmation gap and destroy the cross-session correlation the field exists for. Written down instead of taken silently. |
| **LLM10 (see also)** | Monitoring integrity | **Mitigated 2026-09-01.** `COUNCIL_LOG_LEVEL` is read from the real environment and used to be handed straight to `setLevel()`: an unknown value raised `ValueError` and killed the process, and a valid-but-high value silenced every telemetry record — the run succeeds, the log says nothing, and `scripts/langfuse_check.py` reads an empty file and reports the council emitted nothing. It is now an allow-list capped at INFO, because every record is emitted at INFO and anything above it is a silencer rather than a verbosity setting. `tests/test_reported_debt.py::TestTheLogLevelCannotBeWeaponised`. |
...
| **LLM05** | Improper Output Handling | **Mitigated.** Model output is printed as text and never executed: no `eval`, no shell, no SQL, no HTML rendering. *"Printed as text" had one consumer where it was false, found 2026-08-31:* in GitHub Actions a log line starting at column 0 with `::` **is a command to the runner** — `::error::`, `::add-mask::`, `::stop-commands::` — and the weekly E2E prints three answers, three rankings and a synthesis into a public log. The answers cannot be sanitised without destroying the tool's own output, so the command channel is closed around them instead: `e2e.yml` and `probe.yml` wrap their runs in `::stop-commands::` with a token from `openssl rand`, unique per run as GitHub's documentation requires — verified by `tests/test_workflow_fences.py`, which also pins the `:?` guard against an empty token and the traps that close the fence when a step is killed. Error messages, which are diagnostics rather than product, are additionally flattened to one line in `OpenRouterError` (`tests/test_client.py::TestErrorTextCannotForgeLines`) — they also reach `council.stderr`, which `scripts/langfuse_check.py` parses one JSON object per line. *The fence had no test until 2026-08-31: two independent reviews said the same thing, that the sanitising half was gated and the half protecting the answers was fifteen lines of YAML anyone could delete with every check green.* Stage 2 rankings are regex-validated and a non-matching output is flagged invalid, never guessed — verified by `test_unparseable_output_is_flagged_not_guessed`. |
| **LLM06** | Excessive Agency | **Mitigated.** No autonomous rerun, no self-escalation, no config rewriting. The CLI runs once per invocation with an explicit question. Absorbs 2023's Insecure Plugin Design: the Claude Code skill invokes only the local module, with no external download or execution. **`--env` is bounded** (2026-07-26): the path is resolved and must be a regular file under 64KB, and **only `OPENROUTER_API_KEY` is imported** — one key, because one key is what the program reads. Before this, any `KEY=VALUE` file on disk could be read into the process environment — and since a model assembles this tool's arguments, that path was model-influenced. Covered by `tests/test_cli.py::TestEnvPathIsBounded`. *Corrected 2026-08-31: this said "four allowlisted keys" while the set held three `LANGFUSE_*` names no line of code ever read. A count in prose has no gate behind it — `test_security_doc.py` checks that a cited test exists, never that a number is still true — so the assertion names the key instead.* |
